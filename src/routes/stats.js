...
async function getGameGenres(appId) {
    ...
    try {
        const res = await fetch(`${STORE_API_BASE}/appdetails?appids=${appId}&l=spanish`);
        ...
        const gameDoc = {
            ...
            price: appData.price_overview?.final ? appData.price_overview.final / 100 : 0,
...
router.get('/time/:steamId', async (req, res) => {
    try {
        const apiKey = getSteamApiKey();
        if (!apiKey) return res.status(503).json({ error: 'Steam API key not configured' });

        const games = await fetchOwnedGames(req.params.steamId);
        if (games.length === 0) {
            return res.json({ totalHours: 0, topGame: null, gamesPlayed: 0, gamesOwned: 0 });
        }
        ...
        res.json({
            ...
            topGame: {
                name: top.name,
                hours: topHours,
                percentOfTotal: totalHours > 0 ? Math.round((topHours / totalHours) * 100) : 0,
                appId: top.appid,
            },
            gamesPlayed,
...
router.get('/genres/:steamId', async (req, res) => {
    try {
        ...
        const GENRE_COLORS = {
            'Acción': '#ef4444', 'Action': '#ef4444',
            'Aventura': '#f59e0b', 'Adventure': '#f59e0b',
            'RPG': '#8b5cf6',
            'Estrategia': '#3b82f6', 'Strategy': '#3b82f6',
            'Simuladores': '#10b981', 'Simulation': '#10b981',
            'Indie': '#ec4899',
            'Casual': '#06b6d4',
            'Carreras': '#f97316', 'Racing': '#f97316',
            'Deportes': '#84cc16', 'Sports': '#84cc16',
            'Multijugador masivo': '#6366f1', 'Massively Multiplayer': '#6366f1',
            'Terror': '#991b1b', 'Horror': '#991b1b',
            'Disparos en primera persona': '#dc2626', 'FPS': '#dc2626',
        };
...
router.get('/achievements/:steamId', async (req, res) => {
    try {
        ...
        const games = await fetchOwnedGames(req.params.steamId);
        if (games.length === 0) {
            return res.json({ completionRate: 0, perfectGames: 0, totalGamesPlayed: 0, totalAchievements: 0, rarestAchievement: null });
        }
                ...
                if (achievements.length > 0) {
                    ...
                    try {
                        ...
                        for (const ach of unlocked) {
                            ...
                            if (!rarestAchievement || globalPercent < rarestAchievement.globalPercent) {
                                rarestAchievement = {
